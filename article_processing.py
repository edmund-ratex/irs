...
def start():
    try:    
        artiles = [article1, article2, article3]
        for arti in artiles:             
            splits = arti.split("\n")
            for item in splits:
                if item != "":
            ...
            new_dict = {"title": title, "content":result}    
            row1 = ["title", title]                
            row2 = ["content", result]   
            fileName = regex.sub("_", title)
            filePath = f"./{fileName}.csv"        
            append_to_csv(row1, filePath)                          
            append_to_csv(row2, filePath)                          
        return        
        
        # keywordsList = get_rubydex_glossary()
        keywordsList = read_glossary()        
        jsonContent = []
...
def read_glossary():    
    with open('./tool/misc/rubydex_glossary_2023-03-08.csv', newline='',encoding='utf-8') as csvfile:        
